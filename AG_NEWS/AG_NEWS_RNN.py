...
def collate_batch(batch):
    ...
    for label, text in batch:
        texts.append(text.clone())  # avoid tensor warnings
...
class SmallRNN(nn.Module):
    def __init__(self, vocab_size, embed_dim=50, hidden_dim=64, output_dim=num_classes):
        super().__init__()
...
model = SmallRNN(vocab_size=vocab_size).to(device)
